...
function parseCertInfo(certInfo) {
  let certBuffer = certInfo; // Get a magic constant

  const magic = certBuffer.slice(0, 4).readUInt32BE(0);
  certBuffer = certBuffer.slice(4); // Determine the algorithm used for attestation

  const typeBuffer = certBuffer.slice(0, 2);
  certBuffer = certBuffer.slice(2);
  const type = constants_1.TPM_ST[typeBuffer.readUInt16BE(0)]; // The name of a parent entity, can be ignored
  ...
  certBuffer = certBuffer.slice(qualifiedSignerLength); // Get the expected hash of `attsToBeSigned`

  const extraDataLength = certBuffer.slice(0, 2).readUInt16BE(0);
  certBuffer = certBuffer.slice(2);
  const extraData = certBuffer.slice(0, extraDataLength);
  certBuffer = certBuffer.slice(extraDataLength); // Information about the TPM device's internal clock, can be ignored
  ...
  certBuffer = certBuffer.slice(8); // Attested Name

  const attestedNameLength = certBuffer.slice(0, 2).readUInt16BE(0);
  certBuffer = certBuffer.slice(2);
  const attestedName = certBuffer.slice(0, attestedNameLength);
  certBuffer = certBuffer.slice(attestedNameLength); // Attested qualified name, can be ignored
